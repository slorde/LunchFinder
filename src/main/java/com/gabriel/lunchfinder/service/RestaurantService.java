...
import com.gabriel.lunchfinder.controller.exception.BadRequestException;
...
public class RestaurantService {
	...
	private void validateParameters(Integer rate, Integer distance, Integer price) {
		if (rate > 5 || rate < 1) {
			throw new BadRequestException("Customer Rating should be between 1 and 5");
		}
		
		if (distance > 10 || distance < 1) {
			throw new BadRequestException("Distance should be between 1 mile and 10 miles");
		}
		
		if (price > 50 || price < 10) {
			throw new BadRequestException("Price should be between $10 and $50");
		}
	}
}
